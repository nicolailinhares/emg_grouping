...
for i = 1:L
    ...
    if i > 1 && length(janela) == length(janelas{i-1})
            dados.similarity = executa_struct(dados.similarity, janela, janelas{1});
    else
            dados.similarity = executa_struct(dados.similarity, janela, janela);
    end
